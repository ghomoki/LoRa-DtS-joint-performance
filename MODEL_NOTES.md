...
- **Coding rate fixed at 4/5; coding gain not modelled.** Increasing CR (more FEC) gives real-world *coding gain* (Semtech AN1200.22 quotes ~0.5-1 dB at CR=2, up to ~2-3 dB at CR=4 over the CR=1 baseline). Our `D_SNR_dB(SF)` threshold from the SX127x datasheet implicitly assumes CR=4/5, so letting CR vary would only reduce bit rate without giving back any link margin — making it a "free knob" that the model can't honestly trade. We therefore fix CR at 4/5 throughout. Effective bit rate is `R_b = (SF − 2·LDRO) · B · (4/5) / 2^SF` bits per second; LDRO reduces the per-symbol bit count by 2 (the standard LoRa LDRO encoding), so cells with LDRO on have ~17-20% lower bit rate than their LDRO-off equivalents. Modelling SF-dependent coding gain so that CR can vary as a design parameter is listed under *Optional future additions*.
...
- **Variable coding rate with SF-dependent coding gain.** Currently `CR` is fixed at 4/5 because the `D_SNR_dB(SF)` table assumes that value and we don't model the coding-gain offset for other CR settings. To genuinely treat CR as a design knob, the SNR threshold needs to become `D_SNR_dB(SF, CR) = D_SNR_dB(SF) - coding_gain(SF, CR)`, with the coding gain calibrated from Semtech AN1200.22 or empirical receiver tests. This adds a third dimension to the design space (SF × B × CR) and makes the adaptive-coding-rate analysis possible (LoRaWAN ADR uses CR as a per-link knob). Until then, the model corresponds to LoRaWAN with CR pinned at its default.
